...
export const stylesPlugins = [
  // TailwindCSS
  {
    ...
    order: 'post',
    transform(html, file) {
      if (file.endsWith('head/_head.js') && templateConfig.styles.tailwindcss) {
        return html + `import "@styles/libs/tailwind.css"`;
      }
    },
  },
  ...
  ...(isProduction && templateConfig.styles.groupCssMedia
    ? [
        {
    ...
    ? [
        {
          ...
          writeBundle: {
            ...
            handler: ({ dir }) => {
              ...
              cssCode = cssCode.replace(
                /src:\s*url\(\s*fonts/gi,
                `src:url(${templateConfig.server.path === './' ? '../' : '/'}assets/fonts`
              );
  ...
  // Створення копії файлу(лів) для розробників
...
// Повідомлення
templateConfig.styles.tailwindcss ? logger(`TailwindCSS is done!`) : null;
