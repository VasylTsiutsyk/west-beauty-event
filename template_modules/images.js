              ...
              if (templateConfig.images.optimize.enable) {
                ...
                for (const file of files) {
                  ...
                  if (file.endsWith('.html') || file.endsWith('.js')) {
                    const attrIgnore =
                      templateConfig.images.optimize.attrignore;
  ...
  if (mode === 'html' || mode === 'js') {
    ...
    for (const size of sizes) {
      ...
      const dpiSizesImages = dpi.length
        ? getDpi(+size, dpi, image, imageoutSize, extType, imageoutExt)
        : null;
      templete += `<source media="(max-width: ${size}px)" srcset="${
        dpiSizesImages
          ? dpiSizesImages
          : imageResize(+size, image, imageoutSize, extType)
      }" type="image/${imageoutExt}">`;
    }

    const dpiImages = dpi.length
      ? getDpi(null, dpi, image, imageOutUrl, extType, imageoutExt)
      : null;

    if (templateConfig.images.optimize.modernformat.only || isWebpAvif) {
      templete += `<img ${attr} src="${imageResize(null, image, imageOutUrl, extType)}" ${
      ...
      !isWebpAvif
        ? (templete += `<source srcset="${
            dpiImages
              ? dpiImages
              : imageResize(null, image, imageOutUrl, extType)
          }" type="image/${imageoutExt}">`)
        : null;
...
function imageResize(size, image, imageout, extType) {
  ...
  if (
    ...
    sharp(image, { animated: true })
      .resize(size)
      .png({ quality: templateConfig.images.optimize.png.quality || 80 })
      .toFile(imageout, err => {
        err ? console.error(err) : null;
      });
    ...
    sharp(image, { animated: true })
      .resize(size)
      .jpeg({ quality: templateConfig.images.optimize.jpeg.quality || 80 })
      .toFile(imageout, err => {
        err ? console.error(err) : null;
      });
  } else {
...
function returnUrl(data, url, uniqImages) {
  ...
  if (fs.existsSync(`src/${url}`)) {
    uniqImages.add(url);
    const imageLine = imageResizeInit(
      `src/${url}`,
      [],
      [],
      url.split('.').pop(),
...
function getDpi(size, dpi, image, imageOutUrl, extType, imageoutExt) {
  ...
  for (const dpiItem of dpi) {
    ...
    dpiImages.push(
      `${imageResize(dpiSize, image, newImageOutUrl, extType)} ${dpiItem}x`
    );
  }
